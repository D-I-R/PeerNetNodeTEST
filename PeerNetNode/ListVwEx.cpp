// ListVwEx.cpp                                            (c) DIR, 2019
//
//         Реализация класса CListViewEx - Расширение CListView
//
//
...
#include "PeerNetDlg.h" //#include "resource.h" "afxwin.h" "ListVwEx.h"
...
// Установить режим выделения строки списка через все колонки
BOOL  CListViewEx::SetFullRowSel(BOOL bFullRowSel)
{
  ...
  return bRet;
} // CListViewEx::SetFullRowSel

// Дать индекс выделенной строки списка
int  CListViewEx::GetSelectedRowIndex()
{
  CListCtrl &ListCtrl = GetListCtrl();
  POSITION  pos = ListCtrl.GetFirstSelectedItemPosition();
  return ListCtrl.GetNextSelectedItem(pos);
} // CListViewEx::GetSelectedRowIndex

// Дать строковое значение поля (столбца) в выделенной строке списка
CString & CListViewEx::GetSelectedRowField(short iField)
{
  CListCtrl &ListCtrl = GetListCtrl();
  POSITION  pos = ListCtrl.GetFirstSelectedItemPosition();
  ...
  static CString  s;
  if (iSel >= 0)
    s = ListCtrl.GetItemText(iSel, (int)iField); //nItem, nColumn
  return s;
} // CListViewEx::GetSelectedRowField
...
//protected:  virtual
// Отобразить строку списка (все столбцы)
...
} // CListViewEx::DrawItem

//protected:  virtual
// Перехват обработки сообщения WM_NCDESTROY. Без этого в процессе удаления
//окна CListViewEx лишний раз вызывается delete this, что ПРИВОДИТ К ОШИБКЕ!
void  CListViewEx::PostNcDestroy()
{
	; //ничего не делать!
} // CListViewEx::PostNcDestroy

// Сформировать усечённую строку(когда столбец слишком узкий)
LPCTSTR  CListViewEx::MakeShortString(CDC *pDC, LPCTSTR lpszLong,
									  int nColumnLen, int nOffset)
{
  ...
  return szShort;
} // CListViewEx::MakeShortString

// Перерисовать выделенную строку списка
void  CListViewEx::RepaintSelectedItems()
{
  ...
  // Запомнить индекс выделенной строки ListView:
  //((CDBLoaderDlg *)GetParent())->m_lvSelec = nItem;
...
} // CListViewEx::RepaintSelectedItems
